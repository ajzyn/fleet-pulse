...
describe("parseVehiclesQuery", () => {
  describe("defaults", () => {
    it("returns pageSize=24 and no cursor or filters for an empty URL", () => {
  ...
  describe("valid input", () => {
    it("passes an opaque cursor through untouched", () => {
      expect(parseVehiclesQuery(url("?cursor=eyJvZmZzZXQiOjI0fQ")).cursor).toBe(
        "eyJvZmZzZXQiOjI0fQ",
      );
    });

    it("parses cursor, status and sort together", () => {
      expect(parseVehiclesQuery(url("?cursor=abc&status=active&sort=mileage:asc"))).toEqual({
        cursor: "abc",
        pageSize: 24,
